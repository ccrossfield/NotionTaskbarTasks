...
2. **Discrete buckets over a week horizon, not a continuous gradient**:
   ...
   later / undated (secondary grey, unchanged). All three are custom adaptive
   colours, not the system ones: system yellow is illegible at caption size
   on a light background, and live testing showed system orange and red are
   too bright there as well. Light mode uses deeper crimson/burnt-orange/
   ochre; dark mode brighter tones.
